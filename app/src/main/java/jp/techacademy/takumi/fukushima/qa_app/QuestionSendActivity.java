...
public class QuestionSendActivity extends AppCompatActivity implements View.OnClickListener, DatabaseReference.CompletionListener{
    ...
    public void onClick(View v){
        if(v == mImageView){
            ...
            if(title.length() == 0){
                Snackbar.make(v, "タイトルを入力してください", Snackbar.LENGTH_LONG);
                return;
            }
    ...
    @Override
    public void onRequestPermissionsResult(int requestCode, String permissions[], int[] greanResults){
        switch (requestCode){
            case PERMISSION_REQUEST_CODE:{
                if(greanResults[0] == PackageManager.PERMISSION_GRANTED){
                    //ユーザーが許可したとき
                    showChooser();
                }
                return;
